...
%   the logger object itself will remain alive until those references are released.
    ...
    methods (Static)
        ...
        %   desmostorm.Preferences.set(NAME, VALUE).
            ...
            devMode = desmostorm.Preferences.get("DeveloperMode", false);
            defaults = desmostorm.Log.preferenceDefaults(devMode);

            config = matlabx.config.Logging( ...
                "Level",               desmostorm.Preferences.get("LoggingLevel", defaults.Level), ...
                "Detail",              desmostorm.Preferences.get("LoggingDetail", defaults.Detail), ...
                "SourceDetail",        desmostorm.Preferences.get("LoggingSourceDetail", defaults.SourceDetail), ...
                "CommandWindowLevel",  desmostorm.Preferences.get("LoggingCommandWindowLevel", defaults.CommandWindowLevel), ...
                "CommandWindowDetail", desmostorm.Preferences.get("LoggingCommandWindowDetail", defaults.CommandWindowDetail), ...
                "UILevel",             desmostorm.Preferences.get("LoggingUILevel", defaults.UILevel), ...
                "UIDetail",            desmostorm.Preferences.get("LoggingUIDetail", defaults.UIDetail), ...
                "FileLevel",           desmostorm.Preferences.get("LoggingFileLevel", defaults.FileLevel), ...
                "FileDetail",          desmostorm.Preferences.get("LoggingFileDetail", defaults.FileDetail));
        end
        ...
        %   logger. This is useful for runtime preference toggles.
            ...
            config = desmostorm.Log.configFromPreferences();
            desmostorm.Log.get().configure(config);
        end
        ...
        %   not been explicitly stored.
            ...
            arguments
                devMode (1,1) logical = desmostorm.Preferences.get("DeveloperMode", false)
            end
