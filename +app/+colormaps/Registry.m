classdef Registry
    ...
    % Values are app.colormaps.Colormap objects.
    ...
    methods (Static)
        function refresh()
            ...
            root = app.colormaps.Registry.rootDir();
            if ~isfolder(root)
                app.colormaps.Registry.state('set', S);
                return
            end

            % get colormap categories (names of folders in assets/colormaps)
            cats = utils.getFolderNames(root);
            ...
            for i = 1:numel(cats)
                ...
                for k = 1:numel(mats)
                    ...
                    obj  = app.colormaps.Colormap(name, category, path);

                    % build a new key using the colormap name and category
                    key  = app.colormaps.Registry.mkKey(name, category);
        ...
        function cats = categories()
            S = app.colormaps.Registry.getOrInit();
        ...
        function tf = has(name, category)
            S = app.colormaps.Registry.getOrInit();
            tf = ~isempty(S) && isfield(S,'BYKEY') && isKey(S.BYKEY, app.colormaps.Registry.mkKey(name, category));
        end
        ...
        function obj = get(name, category)
            ...
            if nargin == 1
                ...
                k = app.colormaps.Registry.mkKey(name, category);
        ...
        function p = fileFor(name, category)
            obj = app.colormaps.Registry.get(name, category);
            p = obj.Path;
        end

        function cat = categoryOf(name)
            obj = app.colormaps.Registry.get(name); % must be unique
            cat = obj.Category;
        end
    ...
    methods (Static, Access=private)
        function root = rootDir()
            root = app.Paths.assets('colormaps');
        end
        ...
        function S = getOrInit()
            S = app.colormaps.Registry.state('get');
            if isempty(S)
                app.colormaps.Registry.refresh();
                S = app.colormaps.Registry.state('get');
            end
        end
    end
